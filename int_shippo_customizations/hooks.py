...
BOXES = [
    # xmlid, name, L, W, H (in), empty lb, max lb
...
def _box_values(env, length_in, width_in, height_in, empty_lb, max_lb):
    return {
...
def ensure_package_types(env):
    ...
    for xmlid, name, length_in, width_in, height_in, empty_lb, max_lb in BOXES:
        ...
        vals = _box_values(env, length_in, width_in, height_in, empty_lb, max_lb)
        if existing and existing.exists():
            if not (existing.packaging_length and existing.width and existing.height):
                existing.write(vals)
            continue
